...
import (
	...
	"generate-container/internal/querybuilder"
)
...
func invoke(ctx context.Context, parentJSON []byte, parentName string, fnName string, inputArgs map[string][]byte) (any, error) {
	...
	case "Container":
		...
		case "Generate":
			...
			return (*Container).Generate(&parent, ctx, prompt)
		case "GenerateBashScript":
			...
			return (*Container).GenerateBashScript(&parent, ctx, prompt)
		default:
		...
		return dag.CurrentModule().
			WithObject(
				dag.TypeDef().WithObject("Container").
					WithFunction(
						dag.NewFunction("Generate",
							dag.TypeDef().WithObject("Container")).
							WithArg("prompt", dag.TypeDef().WithKind(Stringkind))).
					WithFunction(
						dag.NewFunction("GenerateBashScript",
							dag.TypeDef().WithObject("Container")).
							WithArg("prompt", dag.TypeDef().WithKind(Stringkind)))), nil
	default:
